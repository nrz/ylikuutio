...
TEST(police_dog_must_be_initialized_appropriately, hirvi_police_dog)
{
    ...
    SymbiosisStruct police_dog_png_symbiosis_struct { Request<Scene>(helsinki_scene), Request(helsinki_pipeline) };
    police_dog_png_symbiosis_struct.model_file_format = "fbx";
    ...
    police_dog_png_police_dog_struct1.initial_rotate_vectors = { glm::vec3(0.0f, 1.0f, 1.0f) };
    police_dog_png_police_dog_struct1.initial_rotate_angles = { static_cast<float>(std::numbers::pi) };
    police_dog_png_police_dog_struct1.original_scale_vector = glm::vec3(1.0f, 1.0f, 1.0f);
    police_dog_png_police_dog_struct1.cartesian_coordinates = CartesianCoordinatesModule(85.00f, 30.00f, 160.00f);
    LocomotionModuleStruct dog_walk_struct(1.0f); // 3.6 km/h.
    LocomotionModuleStruct dog_run_struct(5.0f);
    LocomotionModuleStruct dog_canter_struct(10.0f);
    LocomotionModuleStruct dog_gallop_struct(12.5f); // 45 km/h.
    ...
    auto* const police_dog1 = hirvi_application.get_entity_factory().create_holobiont_derivative<
        PoliceDog,
        hirvi::core::PoliceDogMemoryAllocator,
        HirviScene,
        PoliceDogStruct>(
        hirvi::data::POLICE_DOG,
        Request(helsinki_scene),
        police_dog_png_police_dog_struct1,
        helsinki_non_emancipated_control_center->get_generic_master_module<Movable>(),
        dog_walk_struct,
        dog_run_struct,
        dog_canter_struct,
        dog_gallop_struct);
    ASSERT_NE(police_dog1, nullptr);
    ASSERT_EQ(reinterpret_cast<uintptr_t>(police_dog1) % alignof(hirvi::ontology::PoliceDog), 0);

    ASSERT_FALSE(hirvi_application.has_memory_allocator(hirvi::data::HOLOBIONT));
    ASSERT_TRUE(hirvi_application.has_memory_allocator(hirvi::data::POLICE_DOG));
    ASSERT_TRUE(hirvi_application.has_memory_allocator(hirvi::data::BIONT));

    const GenericParentModule* const parent_of_bionts = police_dog1->get_generic_parent_module<Biont>();
    ...
    ASSERT_EQ(helsinki_scene->get_number_of_non_variable_children(),
              7); // Default `Camera`, `MovableController`, `Pipeline`, `Symbiosis`, 2 `PoliceControlCenter`s, `police_dog1`
    ...
    ASSERT_EQ(police_dog1->get_type(), "hirvi::ontology::PoliceDog*");
    ASSERT_TRUE(police_dog1->get_can_be_erased());
    ASSERT_EQ(police_dog1->get_scene(), helsinki_scene);
    ASSERT_EQ(police_dog1->get_parent(), helsinki_scene);
    ASSERT_EQ(police_dog1->get_number_of_non_variable_children(), 5); // 5 `Biont`s.

    // Even though `PoliceCar` is derived from `Holobiont`. `PoliceCar` should be allocated using its own allocator.
    yli::memory::MemoryAllocator<PoliceDog*>& police_dog_memory_allocator =
            hirvi_application.get_memory_allocator<PoliceDog*>(hirvi::data::POLICE_DOG);
    // `PoliceCar` should be allocated using its own allocator.
    ASSERT_EQ(police_dog_memory_allocator.get_number_of_storages(), 1);
    ASSERT_EQ(police_dog_memory_allocator.get_number_of_instances(), 1);
    ...
    const yli::memory::ConstructibleModule police_dog1_constructible_module =
            police_dog1->get_constructible_module();
    ASSERT_EQ(police_dog1_constructible_module.storage_i, 0);
    ASSERT_EQ(police_dog1_constructible_module.slot_i, 0);
    ASSERT_EQ(police_dog1_constructible_module.alive, true);
}
