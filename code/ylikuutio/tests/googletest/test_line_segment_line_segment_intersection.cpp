...
TEST(line_segment2D_line_segment2D_intersection, parallel_line_segments)
{
    ...
    // |............
    // +----------->
    const ylikuutio::geometry::LineSegment2D line_segment_a = ylikuutio::geometry::LineSegment2D(glm::vec2(2.0f, 2.0f), glm::vec2(11.0f, 5.0f));
    const ylikuutio::geometry::LineSegment2D line_segment_b = ylikuutio::geometry::LineSegment2D(glm::vec2(7.0f, 6.0f), glm::vec2(4.0f, 5.0f));

    const glm::vec2 intersection_point_a_a = ylikuutio::geometry::get_intersection_point(&line_segment_a, &line_segment_a);
    ASSERT_TRUE(std::isnan(intersection_point_a_a.x));
    ASSERT_TRUE(std::isnan(intersection_point_a_a.y));

    const glm::vec2 intersection_point_a_b = ylikuutio::geometry::get_intersection_point(&line_segment_a, &line_segment_b);
    ASSERT_TRUE(std::isnan(intersection_point_a_b.x));
    ASSERT_TRUE(std::isnan(intersection_point_a_b.y));

    const glm::vec2 intersection_point_b_a = ylikuutio::geometry::get_intersection_point(&line_segment_b, &line_segment_a);
    ASSERT_TRUE(std::isnan(intersection_point_b_a.x));
    ASSERT_TRUE(std::isnan(intersection_point_b_a.y));

    const glm::vec2 intersection_point_b_b = ylikuutio::geometry::get_intersection_point(&line_segment_b, &line_segment_b);
    ...
    const ylikuutio::geometry::LineSegment2D line_segment_c = ylikuutio::geometry::LineSegment2D(glm::vec2(11.0f, 5.0f), glm::vec2(2.0f, 2.0f));

    const glm::vec2 intersection_point_a_c = ylikuutio::geometry::get_intersection_point(&line_segment_a, &line_segment_c);
    ASSERT_TRUE(std::isnan(intersection_point_a_c.x));
    ASSERT_TRUE(std::isnan(intersection_point_a_c.y));

    const glm::vec2 intersection_point_b_c = ylikuutio::geometry::get_intersection_point(&line_segment_b, &line_segment_c);
    ASSERT_TRUE(std::isnan(intersection_point_b_c.x));
    ASSERT_TRUE(std::isnan(intersection_point_b_c.y));

    const glm::vec2 intersection_point_c_c = ylikuutio::geometry::get_intersection_point(&line_segment_c, &line_segment_c);
    ASSERT_TRUE(std::isnan(intersection_point_c_c.x));
    ASSERT_TRUE(std::isnan(intersection_point_c_c.y));

    const glm::vec2 intersection_point_c_a = ylikuutio::geometry::get_intersection_point(&line_segment_c, &line_segment_a);
    ASSERT_TRUE(std::isnan(intersection_point_c_a.x));
    ASSERT_TRUE(std::isnan(intersection_point_c_a.y));

    const glm::vec2 intersection_point_c_b = ylikuutio::geometry::get_intersection_point(&line_segment_c, &line_segment_b);
...
TEST(line_segment2D_line_segment2D_intersection, intersecting_line_segments)
{
    ...
    // +----------->
    const ylikuutio::geometry::LineSegment2D line_segment_a = ylikuutio::geometry::LineSegment2D(glm::vec2(5.0f, 1.0f), glm::vec2(10.0f, 6.0f));
    ...
    const ylikuutio::geometry::LineSegment2D line_segment_b = ylikuutio::geometry::LineSegment2D(glm::vec2(3.0f, 5.0f), glm::vec2(9.0f, 2.0f));
    ...
    const glm::vec2 intersection_point = ylikuutio::geometry::get_intersection_point(&line_segment_a, &line_segment_b);
...
TEST(line_segment2D_line_segment2D_intersection, potentially_but_not_intersecting_line_segments)
{
    ...
    const ylikuutio::geometry::LineSegment2D line_segment_b = ylikuutio::geometry::LineSegment2D(glm::vec2(3.0f, 5.0f), glm::vec2(5.0f, 4.0f));
...
TEST(line_segment2D_line_segment2D_intersection, intersection_at_other_lines_point2)
{
    ...
    const ylikuutio::geometry::LineSegment2D line_segment_b = ylikuutio::geometry::LineSegment2D(glm::vec2(3.0f, 5.0f), glm::vec2(7.0f, 3.0f));
...
TEST(line_segment2D_line_segment2D_intersection, intersection_at_both_lines_point2)
{
    ...
    // |...a1....... x = intersection point at (7, 3).
    // +----------->
    const ylikuutio::geometry::LineSegment2D line_segment_a = ylikuutio::geometry::LineSegment2D(glm::vec2(5.0f, 1.0f), glm::vec2(7.0f, 3.0f));
