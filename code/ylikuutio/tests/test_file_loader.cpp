...
{
    std::optional<std::vector<std::uint8_t>> file_content = yli::file::binary_slurp("this_file_does_not_exist_at_all");
    ASSERT_FALSE(file_content);
}
...
TEST(file_must_be_slurped_appropriately, kongtext_svg)
{
    ...
    const std::uint32_t length_of_first_line_of_kongtext_svg = first_line_of_kongtext_svg.size();
    ASSERT_EQ(file_content->compare(0, length_of_first_line_of_kongtext_svg, first_line_of_kongtext_svg), 0);

    const std::uint32_t offset_of_first_character_of_tenth_line = 0x126;
    const std::string tenth_line_of_kongtext_svg = "<font id=\"KongtextRegular\" horiz-adv-x=\"1024\" >";
    const std::uint32_t length_of_tenth_line_of_kongtext_svg = tenth_line_of_kongtext_svg.size();
...
TEST(file_must_be_slurped_appropriately, test3x3_png)
{
    ...
    std::optional<std::vector<std::uint8_t>> file_content = yli::file::binary_slurp(png_filename);
    ASSERT_TRUE(file_content);
    ASSERT_EQ(file_content->size(), 229);
    const std::uint16_t* file_content_uint16_t = (std::uint16_t*) file_content->data();
