...
- [ ] HIRVI/AI Implement `hirvi::MooAbility` placeholder class that inherits `Ability`.
- [ ] HIRVI Replace `Cat`/`Elk`/`PoliceCar`/`PoliceDog`/`PoliceHelicopter`/`PoliceHorse`/`PoliceTrain`/`PoliceTram`/`Train` instances with `Holobiont` instances.
