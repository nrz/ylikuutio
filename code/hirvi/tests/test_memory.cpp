...
TEST(memory_system_must_be_initialized_appropriately, hirvi_application)
{
    ...
    const auto& memory_system = dynamic_cast<yli::memory::MemorySystem<hirvi::data::Datatype>&>(
    ...
    {
        if (memory_system.has_allocator(static_cast<hirvi::data::Datatype>(datatype)))
        {
            n_allocators++;
