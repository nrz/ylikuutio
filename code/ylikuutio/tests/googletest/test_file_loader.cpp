...
TEST(file_must_be_slurped_appropriately, test3x3_bmp)
{
    ...
    const uint32_t magic_number_and_file_size = ylikuutio::memory::read_nonaligned_32_bit<uint8_t, uint32_t>(file_content.data(), 0);
    ASSERT_EQ(magic_number_and_file_size, 0x9e4d42);

    const int32_t image_width = ylikuutio::memory::read_nonaligned_32_bit<uint8_t, int32_t>(file_content.data(), 0x12); // bitmap width in pixels (`int32_t`).
    ASSERT_EQ(image_width, 3);

    const int32_t image_height = ylikuutio::memory::read_nonaligned_32_bit<uint8_t, int32_t>(file_content.data(), 0x16); // bitmap height in pixels (`int32_t`).
    ASSERT_EQ(image_height, 3);

    const uint32_t image_size = ylikuutio::memory::read_nonaligned_32_bit<uint8_t, uint32_t>(file_content.data(), 0x22); // image size.
