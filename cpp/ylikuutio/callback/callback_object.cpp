...
namespace callback_system
{
    ...
    datatypes::AnyValue* CallbackObject::execute()
    {
        for (uint32_t child_i = 0; child_i < this->callback_parameter_pointer_vector.size(); child_i++)
        {
            ...
            {
                this->parent_pointer->set_any_value(input_parameter_pointer->name, input_parameter_pointer->any_value);
            }
        }

        if (this->callback != nullptr)
        {
            return this->callback(this->parent_pointer, this->callback_parameter_pointer_vector);
        }
    }
}
