...
TEST(line2D_line2D_intersection, parallel_lines)
{
    ...
    // |............
    // +----------->
    const geometry::Line2D line_a = geometry::Line2D(glm::vec2(2.0f, 2.0f), glm::vec2(11.0f, 5.0f));
    const geometry::Line2D line_b = geometry::Line2D(glm::vec2(7.0f, 6.0f), glm::vec2(4.0f, 5.0f));

    const glm::vec2 intersection_point_a_a = geometry::get_intersection_point(&line_a, &line_a);
    ASSERT_TRUE(std::isnan(intersection_point_a_a.x));
    ASSERT_TRUE(std::isnan(intersection_point_a_a.y));

    const glm::vec2 intersection_point_a_b = geometry::get_intersection_point(&line_a, &line_b);
    ASSERT_TRUE(std::isnan(intersection_point_a_b.x));
    ASSERT_TRUE(std::isnan(intersection_point_a_b.y));

    const glm::vec2 intersection_point_b_a = geometry::get_intersection_point(&line_b, &line_a);
    ASSERT_TRUE(std::isnan(intersection_point_b_a.x));
    ASSERT_TRUE(std::isnan(intersection_point_b_a.y));

    const glm::vec2 intersection_point_b_b = geometry::get_intersection_point(&line_b, &line_b);
    ...
    const geometry::Line2D line_c = geometry::Line2D(glm::vec2(11.0f, 5.0f), glm::vec2(2.0f, 2.0f));

    const glm::vec2 intersection_point_a_c = geometry::get_intersection_point(&line_a, &line_c);
    ASSERT_TRUE(std::isnan(intersection_point_a_c.x));
    ASSERT_TRUE(std::isnan(intersection_point_a_c.y));

    const glm::vec2 intersection_point_b_c = geometry::get_intersection_point(&line_b, &line_c);
    ASSERT_TRUE(std::isnan(intersection_point_b_c.x));
    ASSERT_TRUE(std::isnan(intersection_point_b_c.y));

    const glm::vec2 intersection_point_c_c = geometry::get_intersection_point(&line_c, &line_c);
    ASSERT_TRUE(std::isnan(intersection_point_c_c.x));
    ASSERT_TRUE(std::isnan(intersection_point_c_c.y));

    const glm::vec2 intersection_point_c_a = geometry::get_intersection_point(&line_c, &line_a);
    ASSERT_TRUE(std::isnan(intersection_point_c_a.x));
    ASSERT_TRUE(std::isnan(intersection_point_c_a.y));

    const glm::vec2 intersection_point_c_b = geometry::get_intersection_point(&line_c, &line_b);
...
TEST(line2D_line2D_intersection, intersecting_lines)
{
    ...
    // +----------->
    const geometry::Line2D line_a = geometry::Line2D(glm::vec2(5.0f, 1.0f), glm::vec2(10.0f, 6.0f));
    ASSERT_EQ(line_a.determinant, 20.0f);
    ASSERT_EQ(line_a.x1_minus_x2, -5.0f);
    ASSERT_EQ(line_a.y1_minus_y2, -5.0f);

    const geometry::Line2D line_b = geometry::Line2D(glm::vec2(3.0f, 5.0f), glm::vec2(9.0f, 2.0f));
    ASSERT_EQ(line_b.determinant, -39.0f);
    ASSERT_EQ(line_b.x1_minus_x2, -6.0f);
    ASSERT_EQ(line_b.y1_minus_y2, 3.0f);

    const glm::vec2 intersection_point = geometry::get_intersection_point(&line_a, &line_b);
