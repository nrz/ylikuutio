        ...
        while (!this->is_exit_requested && this->input_master != nullptr)
        {
            ...
            if (current_time_in_main_loop - this->last_time_for_display_sync >= (1.0f / this->max_fps))
            {
                ...
                this->current_camera_direction = glm::vec3(
                        cos(this->current_camera_vertical_angle) * sin(-1.0f * this->current_camera_horizontal_angle + PI),
                        sin(this->current_camera_vertical_angle),
                        cos(this->current_camera_vertical_angle) * cos(-1.0f * this->current_camera_horizontal_angle + PI));

                // Right vector.
                this->current_camera_right = glm::vec3(
                        sin(-1.0f * this->current_camera_horizontal_angle + 0.5f * PI),
                        0,
                        cos(-1.0f * this->current_camera_horizontal_angle + 0.5f * PI));
