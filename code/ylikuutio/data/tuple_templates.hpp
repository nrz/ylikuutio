...
namespace yli::data
{
    ...
    auto visit_tuple(TupleType&& tuple, FunctionType&& func)
    {
        ...
        return visit_tuple_helper<TupleType, FunctionType>(std::forward<TupleType>(tuple),
                                                           std::forward<FunctionType>(func), index_seq());
    }
    ...
    {
        using index_seq = std::make_index_sequence<std::tuple_size_v<std::remove_reference_t<TupleType>>>;
        return visit_tuple_with_type_helper<Type, TupleType, FunctionType>(
            std::forward<TupleType>(tuple), std::forward<FunctionType>(func), index_seq());
    }
}
