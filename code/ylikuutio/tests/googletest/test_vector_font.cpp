...
TEST(vector_font_must_be_initialized_and_must_bind_to_material_appropriately, headless_shader_and_material_are_children_of_an_ecosystem)
{
    ...
    ASSERT_EQ(&(vector_font->get_universe()), universe);
    ASSERT_EQ(vector_font->get_scene(), nullptr);
    ASSERT_EQ(vector_font->get_parent(), material);
    ASSERT_EQ(vector_font->get_number_of_non_variable_children(), 0);
}

TEST(vector_font_must_be_initialized_and_must_bind_to_material_appropriately, headless_shader_and_material_are_children_of_scene)
{
    yli::ontology::UniverseStruct universe_struct(yli::render::GraphicsApiBackend::HEADLESS);
    yli::ontology::Universe* const universe = new yli::ontology::Universe(universe_struct);
