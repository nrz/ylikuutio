...
namespace yli::memory
{
    ...
    std::vector<T1> copy_circular_buffer_into_vector(const std::array<T1, DataSize>& buffer, const std::size_t start_i,
                                                     const std::size_t buffer_size)
    {
        ...
        const std::size_t n_values_to_copy_first_part {
            buffer_size <= DataSize - start_i ? buffer_size : DataSize - start_i
        };
    ...
    template<typename T1, std::size_t DataSize>
    void copy_circular_buffer_into_begin(std::array<T1, DataSize>& buffer, const std::size_t start_i,
                                         const std::size_t buffer_size)
    {
        if (buffer_size > buffer.size())
        {
        ...
        for (std::size_t iteration_i = 0, iteration_start_i = start_i; iteration_i < start_i;
             iteration_i++, iteration_start_i--)
        {
